...
import InitialDetailValues from "../schemas/initial-detail-values";

export default function DetailFormComponent({ consolidators, countries }) {
  const {
    formId,
    formField: {
      company,
                ...
                <Grid item xs={12}>
                  <Grid container spacing={3}>
                    <Grid item xs={12} sm={4}>
                      <Autocomplete
                        value={values.consolidator}
                        onChange={(e, consolidatorSelected) =>
                          setFieldValue(
                            consolidator.name,
                            consolidatorSelected ?? null
                          )
                        }
                    ...
                    <Grid item xs={12} sm={4}>
                      <Autocomplete
                        ...
                        onChange={(e, countrySelected) =>
                          setFieldValue(country.name, countrySelected ?? null)
                        }
                        options={countries}
                        isOptionEqualToValue={(option, value) =>
                          option == value
